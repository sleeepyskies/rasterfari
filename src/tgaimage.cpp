...
bool TGAImage::read_tga_file(const char *filename) {
    ...
    // equivalent to dividing by 8, goes from bits to byte per pixel
    ...
    if (3 == header.datatypecode || 2 == header.datatypecode) {
        ...
        // read in the rle compressed data
        if (!load_rle_data(in)) {
            in.close();
            std::cerr << "an error occured while reading the data\n";
            return false;
    ...
    // check the image pixel ordering
    if (!(header.imagedescriptor & 0x20)) {
        // 0x20 == 0010 0000
        // => if the 5th bit is 0, flip image vertically
        flip_vertically();
    }
    if (header.imagedescriptor & 0x10) {
        // 0x10 == 0001 0000
        // => if the 4th bit is 1, flip image horizontally
        flip_horizontally();
    }
...
bool TGAImage::write_tga_file(const char *filename, bool rle) {
    ...
    // define the header
    TGA_Header header;
    memset((void *) &header, 0, sizeof(header)); // set everything to 0
    header.bitsperpixel = bytespp << 3;          // from bytes to bits per pixel
    header.width        = width;
    ...
    out.close();
    // automatically open file in tev
    if (system("which tev > /dev/null 2>&1") == 0) {
        std::string command = "tev " + std::string(filename); // open the file
    ...
    for (int j = 0; j < height; j++) {
        ...
        while (erry >= (int) height) {
            if (erry >= (int) height << 1) // it means we jump over a scanline
                memcpy(tdata + nscanline + nlinebytes,
                       tdata + nscanline,
                       nlinebytes);
            erry -= height;
